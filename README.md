...
儿童端会把当前窗口的用途、可见比例和计时状态分开显示。属于娱乐或社交并会消耗额度的窗口使用橙色背景突出；工具、工作和学习窗口仍显示为使用中，但不会计入娱乐总时长。

![儿童端实时窗口状态](docs/screenshots/child-live-status.png)

截图文件只用于文档示例。实际显示内容会随当前打开的窗口、AI 分类结果和每日额度变化。
